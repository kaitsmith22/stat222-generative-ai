...
class MET():
    """
    Custom dataset for image inpainting on Met Images.
    """
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        """
            Get item from the Met dataset with masked image and corresponding mask, all returned as PIL images
        """
        # calculate corresponding file number 
        file_index = index % len(self.filename)

        X = PIL.Image.open(os.path.join(self.data_root, self.split, self.filename[file_index]))
        ...
        # multiply by 255 to create white pixels 
        mask = PIL.Image.fromarray(mask*255)

        return X, masked_image, self.transform(mask)
